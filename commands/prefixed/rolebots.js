...
const data = {
  ...
  async code(ctx) {
    ...
    if (!targets.size)
      return ctx.send(`No hay bots a los que ${accion === "add" ? "añadir" : "quitar"} el rol ${role}`);
    ...
    try {
      ...
      for (const [, member] of targets) {
        try {
          accion === "add"
            ? await member.roles.add(role, `${modTag}: rolebots`)
            : await member.roles.remove(role, `${modTag}: rolebots`);
          done++;
        } catch {
          failed++;
        }
